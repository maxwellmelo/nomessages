...
impl MemoryStorage {
    ...
    fn write<const VERSION: u16>(
        ...
        if let Some(mut previous) = values.insert(storage_key, std::mem::take(&mut *value)) {
            previous.zeroize();
        }
        Ok(())
    }
    ...
    fn append<const VERSION: u16>(
        ...
        list.push(std::mem::take(&mut *value));
    ...
    fn read_list<const VERSION: u16, V: Entity<VERSION>>(
        ...
        let value = Zeroizing::new(match values.get(&storage_key) {
            Some(list_bytes) => serde_json::from_slice::<Vec<Vec<u8>>>(list_bytes)?,
            None => vec![],
        });
    ...
    fn delete<const VERSION: u16>(
        ...
        if let Some(mut removed) = values.remove(&storage_key) {
            removed.zeroize();
        }
...
// NoMessages local hardening: wipe serialized secrets even after lock poisoning.
